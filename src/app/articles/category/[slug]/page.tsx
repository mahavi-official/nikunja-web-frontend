...
export async function generateMetadata({
  ...
  const category = (await getCategories("ARTICLE")).find((item) => item.slug === slug);

  if (!category) return { title: "Not found", robots: { index: false, follow: false } };

  const { meta } = await listPosts({ placement: "ARTICLE", category: slug, page });
  const base = `/articles/category/${slug}`;

  return buildPageMetadata({
    title: category.metaTitle || (page > 1 ? `${category.name} — page ${page}` : `${category.name} articles`),
    description:
      category.metaDescription ||
      category.description ||
      `Articles filed under ${category.name}.`,
    path: page > 1 ? `${base}?page=${page}` : base,
    // Thin taxonomy pages are kept out of the index deliberately.
...
export default async function ArticleCategoryPage({
  ...
  const { posts, meta } = await listPosts({ placement: "ARTICLE", category: slug, page });
